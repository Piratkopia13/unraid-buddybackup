...
- `run-generic-smoke.ps1`: standalone push/pull smoke against a generic OpenZFS host (Debian-based, same path as Proxmox). It prepares a source dataset and snapshot on the generic node, runs `generic_host_setup.sh` in both roles plus its `--verify` checklist (receiver role with the default generic user; sender role with a dedicated `genericSendUser` in forced `--sudo-mode yes`, reproducing the TrueNAS SCALE environment where zfs delegation checks do not apply and only the allowlist scope can stop unallowed commands), seeds `remote_generic` and `remote_pull` entries on nodeA, runs both connection tests (each must report its permission verification: `ZFS receive permissions verified` for push, `Send permission verified` for pull), pushes and pulls real syncoid flows through the allowlists, verifies the pushed and pulled datasets, lists snapshots through the remote_generic path, restores a pushed snapshot back to nodeA, and exercises pull-destination pruning: the generic node is seeded with four sanoid-compatible `autosnap_*_hourly` snapshots (the naming a customized TrueNAS naming schema produces; backdated via `zfs snapshot -o creation` when OpenZFS supports it) plus one non-matching snapshot, the pull destination is added to `snapshots.cfg` with `autoprune = yes`, and sanoid must prune the pulled `autosnap_*_hourly` snapshots to the configured retention while leaving the non-matching snapshot untouched. If the OpenZFS build cannot backdate creation times, the scenario switches to retention `hourly = 0` and asserts that every recognized snapshot is pruned. It also runs negative checks: a `never-allowed` dataset that exists on the generic node (with a snapshot) but was never passed to any `--dataset` must be rejected by both connection tests (pull: "not permitted to access" and no `Send permission verified`; push: "Neither dataset ... nor its parent exists" and no `ZFS receive permissions verified`), which is the regression test for unallowed datasets passing the connection test on sudo-mode hosts.
