...
- **Unraid → TrueNAS SCALE / Proxmox VE / generic Linux (OpenZFS 2.2+)**: push backups to any ZFS-enabled host. A one-time setup command (generated directly within the backup entry on Unraid) configures the remote host with a restricted user, a forced-command allowlist scoped to the configured dataset tree and the minimal `zfs allow` delegations.
