...
usage() {
    cat <<'USAGE_EOF'
Usage: generic_host_setup.sh --dataset NAME --pubkey KEY [options]
...
allow grants it manages: datasets no longer requested are revoked. Grants are
tracked in a root-owned state file next to the allowlist. Self-verifying: finishes
with a PASS/FAIL checklist, and --verify runs only the checks.

Setup (creates/updates the restricted user, allowlist and delegations):
  --dataset NAME            repeatable. parent dataset backups are received into
...
while [ $# -gt 0 ]; do
    ...
    case "$1" in
        --role)
            if [ "$2" != "receiver" ]; then
                fail "--role must be 'receiver' (sender/pull mode has been removed; generic hosts push directly to Unraid)"
                exit 1
            fi
            shift 2
            ;;
...
# the host was configured for. The allowlist also supports --dataset CLI flags
# directly (used in Unraid authorized_keys per incoming buddy).
# When dataset scope is provided, the dataset-scoped patterns below are
# narrowed to those datasets (and their child datasets), so a buddy can never
# touch anything outside their configured tree. When a scope is provided but
# lists no valid datasets, every dataset-scoped command is denied. Without any
# scope, historical unscoped patterns are kept.
my $scope_loaded = 0;
my @DATASET_SCOPE;
if (@cli_datasets) {
    $scope_loaded = 1;
    foreach my $cli_ds (@cli_datasets) {
        push @DATASET_SCOPE, $cli_ds
            if $cli_ds =~ /^[A-Za-z0-9_][A-Za-z0-9_ \/-]*$/;
    }
} elsif ( open( my $scope_fh, '<', $0 . '.datasets' ) ) {
    $scope_loaded = 1;
...
if (@DATASET_SCOPE) {
    $ENV{'BUDDY_DATASET'} = join(',', @DATASET_SCOPE);
}
if ($cli_uid =~ /^[A-Za-z0-9_-]+$/) {
    $ENV{'BUDDY_UID'} = $cli_uid;
}
...
my $PIPE = qr/\s*\|\s*/;
my $MBUFFER_CMD = qr/mbuffer (?:-[rR] \d+[kM])? (?:-W \d+ -I [\w.:-]+ )?-q -s \d+[kM] -m (?:[1-9]\d{0,2}[kM]|1024M)/;
my $COMPRESS_CMD = qr/(?:(?:gzip -3|zcat|pigz -(?:\d+|dc)|(?:zstd|zstdmt) -(?:\d+|dc)|xz(?: -d)?|lzop(?: -dfc)?|lz4(?: -dc)?)\s*\|)?/;
my $SAFE_PROP_VAL = qr/(?:'[\w:.\/-]+'|"[\w:.\/-]+"|[\w:.\/-]+)/;
my $SAFE_O_ASSIGN = qr/(?:(?!mountpoint=|setuid=|exec=|readonly=off)[\w:.-]+=$SAFE_PROP_VAL|'(?!mountpoint=|setuid=|exec=|readonly=off)[\w:.-]+=[\w:.\/-]+'|"(?!mountpoint=|setuid=|exec=|readonly=off)[\w:.-]+=[\w:.\/-]+")/ ;
my $SHORTOPTSVALS = qr/(?:-(?!o\b|x\b)[A-Za-z0-9]+(?:\s+[a-z0-9:._=\/-]+)?\s+|-x\s+[a-z0-9:._-]+\s+|-o\s+$SAFE_O_ASSIGN\s+)*/;
...
my @ALLOWED_COMMANDS = (
    ...
    qr/zfs (?:set readonly=on|inherit readonly) $DATASET$REDIRS/,
    qr/zfs (?:umount|unmount) $DATASET$REDIRS/,
    qr/zfs create (?:-u )?(?:-p )?(?:-o $SAFE_O_ASSIGN )*$DATASET$REDIRS/,
...
sub unwrap_command {
    ...
    # Only unwrap when explicitly wrapped in sh -c '...' or sh -c "..."
    if ($cmd =~ /^sh\s+-c\s+'(.*)'$/s) {
        my $inner = $1;
        $inner =~ s/'\\'''/'/g; # unescape fixture '\''
        $inner =~ s/'\\'\''/'/g; # unescape standard POSIX '\''
...
foreach my $command (split /;/, $unwrapped) {
    ...
    if (!$is_allowed) {
        ...
        my $cmd_ok = 0;
        if ($use_sudo) {
            $cmd_ok = (system('/usr/bin/sudo', '-n', '--', '/usr/bin/bash', '-c', $command) == 0);
        } else {
        ...
        if (!$cmd_ok) {
            ...
            if ($command =~ /zfs\s+(?:recv|receive)\b(?!.*-A)/) {
                my $php_script = $ENV{'BUDDYBACKUP_RC_PHP'} // '/usr/local/emhttp/plugins/buddybackup/scripts/rc.buddybackup.php';
                if (-f $php_script) {
                    system($php_script, 'mark_received_backup');
                }
            }
        }
    }
    ...
    if ($log_text) {
        ...
        if (grep { $_ eq 'syslog' } @log) {
            syslog(LOG_INFO, '%s', $log_text);
        }
    }

    if (!$is_allowed && !$dry_run) {
        exit 1;
    }
}
...
# Every setup run records the datasets it granted (one dataset per line) in a
# root-owned state file next to the allowlist. Later runs use it to converge:
# datasets no longer requested are revoked. --revoke removes single datasets;
# --clean uninstalls everything.
...
# Prints validated dataset names from a state file; corrupt or
...
# Applies delegations for every requested dataset and converges the
# tracked grant set: previously granted datasets that are no longer in the
# --dataset list are revoked first, then the requested
...
apply_grants_and_converge() {
    ...
    local -a tracked_ds=() applied_ds=()
    local ds keep i j rev entries=""
    while IFS= read -r ds || [ -n "$ds" ]; do
        [ -n "$ds" ] || continue
        tracked_ds+=("$ds")
...
run_revoke_mode() {
    ...
    while IFS= read -r d; do
        ...
        # Keep the allowlist scope file in sync: a revoked dataset must no
        # longer be reachable through the forced command either. This file
        # holds plain dataset names, so it gets its own filter.
        f="${d}/buddybackup-${USER_NAME}-restrict_zfs.datasets"
        if [ -f "$f" ]; then
            filter_scope_file "$f"
        fi
    done < <(collect_buddybackup_dirs)
