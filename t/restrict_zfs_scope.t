...
subtest 'sender allowlist is narrowed to the configured dataset tree' => sub {
    my $dir = tempdir(CLEANUP => 1);
    my $script = install_allowlist($dir, 'restrict_zfs_send');
    write_scope($dir, 'restrict_zfs_send', "stuff/various\n");
    ...
    assert_allowed('in-scope send estimate', $script, "zfs send -w -nvP $in_snap");
    assert_blocked('out-of-scope send estimate', $script, "zfs send -w -nvP $out_snap");
    assert_allowed('in-scope send pipeline', $script, "zfs send -w $in_snap | zstdmt -3 | mbuffer  -q -s 128k -m 16M");
    assert_blocked('out-of-scope send pipeline', $script, "zfs send -w $out_snap | zstdmt -3 | mbuffer  -q -s 128k -m 16M");
    ...
    assert_blocked('zfs allow for an out-of-scope dataset', $script, "zfs allow $out");
    assert_allowed('zfs allow for the scope dataset', $script, "zfs allow $in");
    assert_allowed('scope-independent resume estimate', $script, 'zfs send  -nvP -t 1234567890123456');
    assert_allowed('scope-independent probes stay usable', $script, 'command -v zfs');
};
...
subtest 'empty or comment-only scope file denies dataset-scoped commands' => sub {
    ...
    write_scope($dir, 'restrict_zfs_send', '');
    ...
    write_scope($dir, 'restrict_zfs_send', "# nothing configured yet\n\n");
    assert_blocked('dataset query denied by comment-only scope', $script, "zfs get -H name 'stuff/various'");
    assert_allowed('scope-independent probe still works with comment-only scope', $script, 'zfs version');
};
...
subtest 'invalid scope lines are skipped; valid ones still apply' => sub {
    ...
    write_scope($dir, 'restrict_zfs_send', "    indented\n'quoted'\nds; rm -rf /\n../evil\n");
